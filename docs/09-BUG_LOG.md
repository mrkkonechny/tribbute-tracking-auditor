...
- **Status:** Resolved — port-based PANEL_OPEN/PANEL_CLOSED replaces isPopupOpen + window.unload (2026-03-26)
- **Severity:** High
- **Date Found:** 2026-03-26
...
- **Fix:** See ROAD-0004 — replaced with `chrome.runtime.connect()` port disconnect via `background.js` relay (PANEL_OPEN/PANEL_CLOSED). `isPopupOpen` flag replaced by port-based `isPanelOpen`.
- **Related:** ROAD-0004
...
- **Status:** Resolved — safeSendMessage now distinguishes transient SW restart errors (2026-03-26)
- **Severity:** High
- **Date Found:** 2026-03-26
- **Date Resolved:** 2026-03-26
...
- **Fix:** See ROAD-0003 — `safeSendMessage` now distinguishes transient errors from fatal errors (`chrome.runtime.id === undefined`). Transient errors skip the send without permanently disabling the content script.
- **Related:** ROAD-0003
...
- **Status:** Resolved — tabs permission added in manifest.json v1.3.0 (2026-03-26)
- **Severity:** High
- **Date Found:** 2026-03-26
- **Date Resolved:** 2026-03-26
...
- **Fix:** See ROAD-0002 — `"tabs"` added to `permissions` in `manifest.json` as part of v1.3.0 side panel migration.
- **Related:** ROAD-0002
